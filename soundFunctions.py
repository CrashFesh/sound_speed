...
def speed_of_sound (t, p, x5): # 1-азот, 2-кислород, 3-аргон, 4-пар, 5-CO2 (x3-молярная концентрация CO2 (в %))
    pa=101325 #атм. давление
    R=8.314 #газ. постоянная
    ui=[28.014*(10**(-3)), 31.998*(10**(-3)), 39.948*(10**(-3)), 18.01*(10**(-3)), 44.01*(10**(-3))] #молярная масса (кг) для 3-х газов
    Cpi=[1.051, 0.913, 0.523, 1.863, 0.838] #Cp 3-х газов
    Cvi=[0.743, 0.658, 0.312, 1.403, 0.649] #Cv 3-х газов
    v=p/(pa) #молярка пара при общей с воздухом молярки = 1
    x4=100*v
    x1235=100-x4
    x1=78.084/100*x1235
    x3=0.934/100*x1235
    x2=x1235-x1-x3-x5
    yi=[x1/100, x2/100, x3/100, x4/100, x5/100] #перевод концентрации из %
    u=0
    for i in range(0, 5): #подсчёт сумарной молярной массы
        u+=ui[i]*yi[i]
    g1=0
    g2=0
    for i in range(0, 5): #подсчёт показателя адиабаты
        g1+=Cpi[i]*ui[i]*yi[i]
        g2+=Cvi[i]*ui[i]*yi[i]
    g=g1/g2
